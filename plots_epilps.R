# ================================================================================
# BEAUTIFUL 2×2 INTERACTIVE GRID - ABM + Cori + WT + RtEstim + EpiLPS (Baseline Only)
# ================================================================================
...
COLORS <- list(
  ...
  EpiLPS  = list(line = "#8E44AD", ribbon = "rgba(142, 68, 173, 0.18)")
)
...
load_epilps <- function(model_type, R0_val, transition) {
  csv_path <- file.path(EPILPS_DIR, paste0(model_type, "_R0_", R0_val, "_", transition, ".csv"))
  rds_path <- file.path(EPILPS_DIR, paste0(model_type, "_R0_", R0_val, "_", transition, ".rds"))
...
prepare_panel_data <- function(model_type, R0_val, transition) {
  ...
  epilps   <- safe_load(load_epilps, model_type, R0_val, transition)
  ...
  # Summarize EpiLPS
  epilps_summary <- NULL
  if (!is.null(epilps) && nrow(epilps) > 0) {
    epilps_summary <- epilps %>%
      filter(date >= DATE_MIN, date <= DATE_MAX) %>%
      group_by(date) %>%
      summarise(
        mean_rt  = mean(median_rt, na.rm = TRUE),
  ...
  list(cw = cw_summary, rtestim = rt_summary, epilps = epilps_summary, abm = abm_summary, R0 = R0_val)
}
...
create_panel <- function(data, R0_val, is_first_panel = FALSE) {
  ...
  # --- EpiLPS ---
  if (!is.null(data$epilps)) {
    p <- p %>%
      add_ribbons(
        data = data$epilps,
        x = ~date, ymin = ~ci_lower, ymax = ~ci_upper,
        fillcolor = COLORS$EpiLPS$ribbon, line = list(width = 0),
        showlegend = FALSE, legendgroup = "EpiLPS", hoverinfo = "skip"
      ) %>%
      add_lines(
        data = data$epilps,
        x = ~date, y = ~mean_rt,
        line = list(color = COLORS$EpiLPS$line, width = 2.8),
        name = "EpiLPS", legendgroup = "EpiLPS",
        showlegend = is_first_panel,
        hovertemplate = "<b>EpiLPS</b><br>Day: %{x}<br>Rt: %{y:.3f}<extra></extra>"
      )
  }
    ...
    layout(
      title = list(
        text = paste0(
          ...
          trans_label, " Transition | ABM + Cori + Wallinga-Teunis + RtEstim + EpiLPS",
          "</span>"
        ),
...
create_all_grids <- function() {
  cat("╔══════════════════════════════════════════════════════════════╗\n")
  cat("║   COMBINED Rt GRID: ABM + Cori + WT + RtEstim + EpiLPS    ║\n")
  cat("╚══════════════════════════════════════════════════════════════╝\n")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .plot-card {
            background: linear-gradient(135deg, #8E44AD 0%, #2C3E50 100%);
            border-radius: 15px; box-shadow: 0 8px 20px rgba(142, 68, 173, 0.3);
            transition: all 0.3s ease; cursor: pointer;
        }
        .plot-card:hover { transform: translateY(-5px); box-shadow: 0 12px 30px rgba(142, 68, 173, 0.5); }
...
<div class="container">
    <div class="header">
        ...
        <p>ABM + Cori + Wallinga-Teunis + RtEstim + EpiLPS</p>
    </div>
    <div class="info">
