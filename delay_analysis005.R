...
analyze_delays_single_sim <- function(saved_data, sim_id = 1, 
                                      ...
                                      delay_scenarios = list("medium_delay" = 0.3,
                                                             "high_delay"   = 0.5),
                                      start_day = 0) {
  cat("Analyzing delays for simulation", sim_id, "\n")
  ...
  for (scenario_name in names(delay_scenarios)) {
    ...
    cat("  Processing", scenario_name, "- delaying", delay_pct*100, "% of cases\n")
    
    delayed_incidence <- apply_reporting_delays(original_incidence, delay_pct)
    if (sum(delayed_incidence) == 0) {
      cat("    No cases after applying delays\n")
      next
    }
...
analyze_delays_all_sims <- function(saved_data, how_infected = "susceptible_to_exposed",
                                    delay_scenarios = list("medium_delay" = 0.3,
                                                           "high_delay"   = 0.5),
                                    max_sims = 60, start_day = 0, verbose = TRUE) {
  if (verbose) {
  ...
  for (i in seq_along(sim_ids)) {
    ...
    sim_results <- analyze_delays_single_sim(
      saved_data, sim_id, how_infected, delay_scenarios, start_day = start_day
    )
    ...
    for (transition in transitions) {
      for (R0_val in R0_values) {
        ...
        tryCatch({
          ...
          cat("🔄 Running delay analysis (SI-based)...\n")
...
test_single_simulation_delays <- function(model_type = "full", R0_val = 2.0, 
                                          ...
                                          start_day = 0) {
  cat("╔══════════════════════════════════════════════════════════════╗\n")
  cat("║                TESTING SINGLE SIMULATION (SI)               ║\n")
  cat("╚══════════════════════════════════════════════════════════════╝\n")
  ...
  medium_delayed <- apply_reporting_delays(test_incidence, 0.3)
  cat("Medium delayed (30%):", paste(medium_delayed, collapse = ", "), "\n")
  high_delayed <- apply_reporting_delays(test_incidence, 0.5)
  cat("High delayed (50%):", paste(high_delayed, collapse = ", "), "\n")
  
  cat("\n🔬 Testing single simulation analysis (SI-based)...\n")
  ...
  if (!is.null(delay_results) && nrow(delay_results) > 0) {
    ...
    cat("\n🎨 Creating test plot...\n")
    plot_obj <- create_combined_delay_plot(delay_results, abm_data, R0_val, model_type, transition)
    test_plot_file <- paste0("test_delay_plot_", model_type, "_R0_", R0_val, "_sim_", sim_id, ".png")
...
quick_diagnosis <- function() {
  ...
  test_results <- test_single_simulation_delays()
  !is.null(test_results)
}

# ── COMPREHENSIVE PLOTS FROM SAVED DELAY RESULTS ────────────────────────────────
# Builds comparison plots using outputs saved by run_complete_delay_analysis()
# Expects:
#   complete_delay_results/delay_results_<model>_R0_<val>_<transition>.rds
# And ABM baselines at:
#   saved_data/<model>_R0_<val>_n_1e+05_nsim_100_rt_ci.rds
