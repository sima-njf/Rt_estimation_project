# =============================================================================
# EpiLPS – REPORTING DELAYS – ALL SCENARIOS (mclapply)
# Uniform(1,7) reporting delays applied before estimation
# =============================================================================

library(EpiLPS)
library(dplyr)
library(parallel)

source("data001.R")   # load_seir_data()

# ── CONFIG ──────────────────────────────────────────────────────────────────────
DEFAULT_SEED <- 12345

# =============================================================================
# Apply reporting delays: Uniform(1,7) to a fraction of cases
# =============================================================================
apply_reporting_delays <- function(incidence_vector, delay_percentage = 0.3) {
  n_days <- length(incidence_vector)
  if (sum(incidence_vector) == 0 || delay_percentage == 0) return(incidence_vector)
  
  max_delay <- 7
  delayed_incidence <- numeric(n_days + max_delay)
  
  for (day in seq_len(n_days)) {
    daily_cases <- incidence_vector[day]
    if (daily_cases > 0) {
      n_delayed <- rbinom(1, daily_cases, delay_percentage)
      n_on_time <- daily_cases - n_delayed
      delayed_incidence[day] <- delayed_incidence[day] + n_on_time
      
      if (n_delayed > 0) {
        delays <- sample(1:7, n_delayed, replace = TRUE)
        for (d in delays) {
          delay_day <- min(day + d, length(delayed_incidence))
          delayed_incidence[delay_day] <- delayed_incidence[delay_day] + 1
        }
      }
    }
  }
  delayed_incidence[1:n_days]
}

# =============================================================================
# Process ONE simulation (with reporting delays)
# =============================================================================
process_single_sim_epilps_delay <- function(
    sim_id,
    transitions_data,
    generation_data,
    transmission_data,
    how_infected,
    delay_percentage = 0.3,
    delay_scenario   = "medium_delay",
    verbose_debug    = FALSE
) {
  
  tryCatch({
    
    # ---- Filter to this simulation ------------------------------------------
    sim_transitions  <- transitions_data  %>% filter(id == sim_id)
    sim_generation   <- generation_data   %>% filter(sim_num == sim_id)
    sim_transmission <- transmission_data %>% filter(sim_num == sim_id)
    
    # ---- Incidence from $transitions (already aggregated) -------------------
    if (how_infected == "susceptible_to_exposed") {
      incidence <- sim_transitions$susceptible_to_exposed
    } else if (how_infected == "exposed_to_infected") {
      incidence <- sim_transitions$exposed_to_infected
    } else {
      return(NULL)
    }
    
    days <- sim_transitions$date
    
    # For S->E: day 0 includes seeds, so remove seed count from day 0
    if (how_infected == "susceptible_to_exposed") {
      n_seeds <- sum(sim_transmission$source == -1)
      incidence[1] <- incidence[1] - n_seeds
      if (incidence[1] < 0) incidence[1] <- 0
    }
    
    # ---- Apply reporting delays ---------------------------------------------
    incidence <- apply_reporting_delays(incidence, delay_percentage)
    
    # ---- Trim leading zeros -------------------------------------------------
    first_nz <- which(incidence > 0)[1]
    if (is.na(first_nz)) return(NULL)
    
    # Trim trailing zeros
    last_nz <- max(which(incidence > 0))
    
    incidence <- incidence[first_nz:last_nz]
    days      <- days[first_nz:last_nz]
    
    if (length(incidence) < 10) return(NULL)
    
    # ---- Generation interval PMF (remove seeds) ----------------------------
    seed_ids <- sim_transmission %>%
      filter(source == -1) %>%
      pull(target)
    
    sim_gen_clean <- sim_generation %>%
      filter(!(source %in% seed_ids))
    
    gen_times <- sim_gen_clean$gentime
    gen_times <- gen_times[!is.na(gen_times) & gen_times > 0]
    
    if (length(gen_times) < 10) return(NULL)
    
    max_gen    <- max(gen_times)
    gen_counts <- table(factor(gen_times, levels = 0:max_gen))
    gen_int    <- as.numeric(gen_counts) / sum(gen_counts)
    
    if (verbose_debug) {
      cat(sprintf(
        "Sim %d | days=%d | total_cases=%d | GI_len=%d | mean_GI=%.1f\n",
        sim_id, length(incidence), sum(incidence),
        length(gen_int), mean(gen_times)
      ))
    }
    
    # ---- Rt estimation (EpiLPS) ---------------------------------------------
    si_spec    <- Idist(probs = gen_int)
    epilps_fit <- estimR(incidence = incidence, si = si_spec$pvec)
    
    # ---- Output (trim early unreliable days) --------------------------------
    burn_in <- ceiling(quantile(gen_times, 0.95))
    
    out <- data.frame(
      sim_id           = sim_id,
      method           = "EpiLPS",
      date             = days,
      median_rt        = epilps_fit$RLPS$Rq0.50,
      q025_rt          = epilps_fit$RLPS$Rq0.025,
      q975_rt          = epilps_fit$RLPS$Rq0.975,
      delay_scenario   = delay_scenario,
      delay_percentage = delay_percentage
    )
    
    out <- out %>% filter(date > burn_in)
    out
    
  }, error = function(e) {
    if (verbose_debug) {
      cat(sprintf("Sim %d ERROR: %s\n", sim_id, e$message))
    }
    NULL
  })
}

# =============================================================================
# Apply to ALL simulations — parallel with mclapply
# =============================================================================
apply_epilps_delay_parallel <- function(
    saved_data,
    how_infected,
    delay_percentage = 0.3,
    delay_scenario   = "medium_delay",
    ncores           = ncores,
    verbose          = TRUE
) {
  
  sim_nums <- unique(saved_data$transitions$id)
  
  if (verbose) {
    cat("Running EpiLPS (delay,", delay_percentage * 100, "%,", delay_scenario,
        ") for", length(sim_nums), "simulations on", ncores, "cores\n")
  }
  
  results <- parallel::mclapply(sim_nums, FUN = function(sid) {
    
    # Set per-simulation seed for reproducible delays
    set.seed(DEFAULT_SEED + sid)
    
    process_single_sim_epilps_delay(
      sim_id            = sid,
      transitions_data  = saved_data$transitions,
      generation_data   = saved_data$generation,
      transmission_data = saved_data$transmission,
      how_infected      = how_infected,
      delay_percentage  = delay_percentage,
      delay_scenario    = delay_scenario,
      verbose_debug     = FALSE
    )
    
  }, mc.cores = ncores)
  
  # Remove NULLs and failed results
  is_valid <- sapply(results, function(x) is.data.frame(x))
  n_errors <- sum(sapply(results, function(x) inherits(x, "try-error") || inherits(x, "error")))
  
  results <- results[is_valid]
  
  if (verbose) {
    cat("  Successful:", length(results), "/", length(sim_nums), "\n")
    if (n_errors > 0) cat("  Errors:", n_errors, "\n")
  }
  
  if (length(results) == 0) return(NULL)
  bind_rows(results)
}

# =============================================================================
# Run ALL MODEL TYPES × R0 × TRANSITIONS × DELAY SCENARIOS
# =============================================================================
process_epilps_delay_all <- function(ncores          = ncores,
                                     delay_scenarios = list("medium_delay" = 0.3,
                                                            "high_delay"   = 0.5)) {
  
  R0_values   <- c(1.5, 2.0, 3.0, 5.0)
  model_types <- c("full", "partial")
  transitions <- c("susceptible_to_exposed", "exposed_to_infected")
  
  dir.create("epilps_delay_results", showWarnings = FALSE)
  
  for (model_type in model_types) {
    
    cat("\n==============================\n")
    cat("MODEL TYPE:", model_type, "\n")
    cat("==============================\n")
    
    for (R0_val in R0_values) {
      
      prefix <- paste0(
        "saved_data/",
        model_type, "_R0_", R0_val, "_n_1e+05_nsim_100"
      )
      
      if (!file.exists(paste0(prefix, "_metadata.rds"))) {
        cat("  skipping:", prefix, "\n")
        next
      }
      
      cat("\n=== R0 =", R0_val, "===\n")
      saved_data <- load_seir_data(prefix)
      
      for (transition in transitions) {
        
        cat("Transition:", transition, "\n")
        
        # Run each delay scenario and combine
        all_scenario_results <- NULL
        
        for (scenario_name in names(delay_scenarios)) {
          delay_pct <- delay_scenarios[[scenario_name]]
          
          scenario_results <- apply_epilps_delay_parallel(
            saved_data,
            how_infected     = transition,
            delay_percentage = delay_pct,
            delay_scenario   = scenario_name,
            ncores           = ncores
          )
          
          if (!is.null(scenario_results)) {
            all_scenario_results <- bind_rows(all_scenario_results, scenario_results)
          }
        }
        
        if (!is.null(all_scenario_results) && nrow(all_scenario_results) > 0) {
          
          out_file <- paste0(
            "epilps_delay_results/",
            model_type, "_R0_", R0_val, "_", transition, ".rds"
          )
          
          saveRDS(all_scenario_results, out_file)
          write.csv(all_scenario_results, sub(".rds", ".csv", out_file), row.names = FALSE)
          
          cat("  saved:", basename(out_file),
              "| sims:", length(unique(all_scenario_results$sim_id)),
              "| scenarios:", paste(unique(all_scenario_results$delay_scenario), collapse = ", "), "\n")
        }
      }
    }
  }
  
  cat("\n=== EpiLPS DELAY ALL SCENARIOS COMPLETE ===\n")
}

# =============================================================================
# RUN
# =============================================================================

process_epilps_delay_all(
  ncores          = 12,
  delay_scenarios = list("medium_delay" = 0.3, "high_delay" = 0.5)
)


# =============================================================================
# RtEstim – REPORTING DELAYS – ALL SCENARIOS (mclapply)
# Uniform(1,7) reporting delays applied before estimation
# =============================================================================

library(rtestim)
library(dplyr)
library(parallel)

source("data001.R")   # load_seir_data()

# ── CONFIG ──────────────────────────────────────────────────────────────────────
DEFAULT_SEED <- 12345

# =============================================================================
# Apply reporting delays: Uniform(1,7) to a fraction of cases
# =============================================================================
apply_reporting_delays <- function(incidence_vector, delay_percentage = 0.3) {
  n_days <- length(incidence_vector)
  if (sum(incidence_vector) == 0 || delay_percentage == 0) return(incidence_vector)
  
  max_delay <- 7
  delayed_incidence <- numeric(n_days + max_delay)
  
  for (day in seq_len(n_days)) {
    daily_cases <- incidence_vector[day]
    if (daily_cases > 0) {
      n_delayed <- rbinom(1, daily_cases, delay_percentage)
      n_on_time <- daily_cases - n_delayed
      delayed_incidence[day] <- delayed_incidence[day] + n_on_time
      
      if (n_delayed > 0) {
        delays <- sample(1:7, n_delayed, replace = TRUE)
        for (d in delays) {
          delay_day <- min(day + d, length(delayed_incidence))
          delayed_incidence[delay_day] <- delayed_incidence[delay_day] + 1
        }
      }
    }
  }
  delayed_incidence[1:n_days]
}

# =============================================================================
# Process ONE simulation (with reporting delays)
# =============================================================================
process_single_sim_rtestim_delay <- function(
    sim_id,
    transitions_data,
    generation_data,
    transmission_data,
    how_infected,
    delay_percentage = 0.3,
    delay_scenario   = "medium_delay",
    verbose_debug    = FALSE
) {
  
  tryCatch({
    
    # ---- Filter to this simulation ------------------------------------------
    sim_transitions  <- transitions_data  %>% filter(id == sim_id)
    sim_generation   <- generation_data   %>% filter(sim_num == sim_id)
    sim_transmission <- transmission_data %>% filter(sim_num == sim_id)
    
    # ---- Incidence from $transitions (already aggregated) -------------------
    if (how_infected == "susceptible_to_exposed") {
      incidence <- sim_transitions$susceptible_to_exposed
    } else if (how_infected == "exposed_to_infected") {
      incidence <- sim_transitions$exposed_to_infected
    } else {
      return(NULL)
    }
    
    days <- sim_transitions$date
    
    # For S->E: day 0 includes seeds, so remove seed count from day 0
    if (how_infected == "susceptible_to_exposed") {
      n_seeds <- sum(sim_transmission$source == -1)
      incidence[1] <- incidence[1] - n_seeds
      if (incidence[1] < 0) incidence[1] <- 0
    }
    
    # ---- Apply reporting delays ---------------------------------------------
    incidence <- apply_reporting_delays(incidence, delay_percentage)
    
    # ---- Trim leading zeros (rtestim needs first value > 0) -----------------
    first_nz <- which(incidence > 0)[1]
    if (is.na(first_nz)) return(NULL)
    
    # Trim trailing zeros (zeros at the tail cause NAs in cv_estimate_rt)
    last_nz <- max(which(incidence > 0))
    
    incidence <- incidence[first_nz:last_nz]
    days      <- days[first_nz:last_nz]
    
    if (length(incidence) < 10) return(NULL)
    
    # ---- Generation interval PMF (remove seeds) ----------------------------
    seed_ids <- sim_transmission %>%
      filter(source == -1) %>%
      pull(target)
    
    sim_gen_clean <- sim_generation %>%
      filter(!(source %in% seed_ids))
    
    gen_times <- sim_gen_clean$gentime
    gen_times <- gen_times[!is.na(gen_times) & gen_times > 0]
    
    if (length(gen_times) < 10) return(NULL)
    
    max_gen    <- max(gen_times)
    gen_counts <- table(factor(gen_times, levels = 0:max_gen))
    gen_int    <- as.numeric(gen_counts) / sum(gen_counts)
    
    if (verbose_debug) {
      cat(sprintf(
        "Sim %d | days=%d | total_cases=%d | GI_len=%d | mean_GI=%.1f\n",
        sim_id, length(incidence), sum(incidence),
        length(gen_int), mean(gen_times)
      ))
    }
    
    # ---- Rt estimation (cross-validated) ------------------------------------
    rtestim_fit <- suppressWarnings(cv_estimate_rt(
      observed_counts = incidence,
      delay_distn     = gen_int,
      nsol            = 20
    ))
    
    # ---- Confidence bands ---------------------------------------------------
    chosen_lambda <- rtestim_fit$lambda.1se
    if (is.na(chosen_lambda) || is.infinite(chosen_lambda) || chosen_lambda < 0) {
      chosen_lambda <- rtestim_fit$lambda.min
    }
    
    rtestim_cb <- suppressWarnings(confband(rtestim_fit, lambda = chosen_lambda))
    
    # ---- Output (trim early unreliable days) --------------------------------
    burn_in <- ceiling(quantile(gen_times, 0.95))
    
    out <- data.frame(
      sim_id           = sim_id,
      method           = "RtEstim",
      date             = days,
      median_rt        = rtestim_cb$fit,
      q025_rt          = rtestim_cb$`2.5%`,
      q975_rt          = rtestim_cb$`97.5%`,
      delay_scenario   = delay_scenario,
      delay_percentage = delay_percentage
    )
    
    out <- out %>% filter(date > burn_in)
    out
    
  }, error = function(e) {
    if (verbose_debug) {
      cat(sprintf("Sim %d ERROR: %s\n", sim_id, e$message))
    }
    NULL
  })
}

# =============================================================================
# Apply to ALL simulations — parallel with mclapply
# =============================================================================
apply_rtestim_delay_parallel <- function(
    saved_data,
    how_infected,
    delay_percentage = 0.3,
    delay_scenario   = "medium_delay",
    ncores           = ncores,
    verbose          = TRUE
) {
  
  sim_nums <- unique(saved_data$transitions$id)
  
  if (verbose) {
    cat("Running RtEstim (delay,", delay_percentage * 100, "%,", delay_scenario,
        ") for", length(sim_nums), "simulations on", ncores, "cores\n")
  }
  
  results <- parallel::mclapply(sim_nums, FUN = function(sid) {
    
    # Set per-simulation seed for reproducible delays
    set.seed(DEFAULT_SEED + sid)
    
    process_single_sim_rtestim_delay(
      sim_id            = sid,
      transitions_data  = saved_data$transitions,
      generation_data   = saved_data$generation,
      transmission_data = saved_data$transmission,
      how_infected      = how_infected,
      delay_percentage  = delay_percentage,
      delay_scenario    = delay_scenario,
      verbose_debug     = FALSE
    )
    
  }, mc.cores = ncores)
  
  # Remove NULLs and failed results
  is_valid <- sapply(results, function(x) is.data.frame(x))
  n_errors <- sum(sapply(results, function(x) inherits(x, "try-error") || inherits(x, "error")))
  
  results <- results[is_valid]
  
  if (verbose) {
    cat("  Successful:", length(results), "/", length(sim_nums), "\n")
    if (n_errors > 0) cat("  Errors:", n_errors, "\n")
  }
  
  if (length(results) == 0) return(NULL)
  bind_rows(results)
}

# =============================================================================
# Run ALL MODEL TYPES × R0 × TRANSITIONS × DELAY SCENARIOS
# =============================================================================
process_rtestim_delay_all <- function(ncores          = ncores,
                                      delay_scenarios = list("medium_delay" = 0.3,
                                                             "high_delay"   = 0.5)) {
  
  R0_values   <- c(5)
  model_types <- c("full")
  transitions <- c("susceptible_to_exposed", "exposed_to_infected")
  
  dir.create("rtestim_delay_results", showWarnings = FALSE)
  
  for (model_type in model_types) {
    
    cat("\n==============================\n")
    cat("MODEL TYPE:", model_type, "\n")
    cat("==============================\n")
    
    for (R0_val in R0_values) {
      
      prefix <- paste0(
        "saved_data/",
        model_type, "_R0_", R0_val, "_n_1e+05_nsim_100"
      )
      
      if (!file.exists(paste0(prefix, "_metadata.rds"))) {
        cat("  skipping:", prefix, "\n")
        next
      }
      
      cat("\n=== R0 =", R0_val, "===\n")
      saved_data <- load_seir_data(prefix)
      
      for (transition in transitions) {
        
        cat("Transition:", transition, "\n")
        
        # Run each delay scenario and combine
        all_scenario_results <- NULL
        
        for (scenario_name in names(delay_scenarios)) {
          delay_pct <- delay_scenarios[[scenario_name]]
          
          scenario_results <- apply_rtestim_delay_parallel(
            saved_data,
            how_infected     = transition,
            delay_percentage = delay_pct,
            delay_scenario   = scenario_name,
            ncores           = ncores
          )
          
          if (!is.null(scenario_results)) {
            all_scenario_results <- bind_rows(all_scenario_results, scenario_results)
          }
        }
        
        if (!is.null(all_scenario_results) && nrow(all_scenario_results) > 0) {
          
          out_file <- paste0(
            "rtestim_delay_results/",
            model_type, "_R0_", R0_val, "_", transition, ".rds"
          )
          
          saveRDS(all_scenario_results, out_file)
          write.csv(all_scenario_results, sub(".rds", ".csv", out_file), row.names = FALSE)
          
          cat("  saved:", basename(out_file),
              "| sims:", length(unique(all_scenario_results$sim_id)),
              "| scenarios:", paste(unique(all_scenario_results$delay_scenario), collapse = ", "), "\n")
        }
      }
    }
  }
  
  cat("\n=== RtEstim DELAY ALL SCENARIOS COMPLETE ===\n")
}

# =============================================================================
# RUN
# =============================================================================

process_rtestim_delay_all(
  ncores          = 12,
  delay_scenarios = list("medium_delay" = 0.3, "high_delay" = 0.5)
)
